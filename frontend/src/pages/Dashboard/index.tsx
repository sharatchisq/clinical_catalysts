...
const getMotivationalMessage = (sectionId: string, hasNegativeAnswers: boolean) => {
  const messages = {
  ...
  return messages[sectionId as keyof typeof messages] || "You're doing great! 🌟";
};
...
const EmojiDisplay = () => {
  ...
  const getCurrentEmoji = () => {
    const currentSectionQuestions = questionnaire.sections
    ...
    if (lastAnswered) {
      const emoji = getEmojiForAnswer(lastAnswered.question.id, lastAnswered.answer);
    ...
    return defaultEmojis[currentSectionId as keyof typeof defaultEmojis] || '📝';
  };
